...
//for(i=0;i<n_cg_sites;i++) printf("%d th cg site had type %d\n",i,cg_types[i]);
    ...
    rvec *fcg=(rvec *)malloc(n_cg_sites*sizeof(rvec));
    while(!read_trr(fp_trr,n_atom,&i_step,&t,&lambda,box,x,NULL,f)) //loop for snapshots
    {
        for(i=0;i<n_cg_sites;i++)//initialization for xcg,fcg, important!
            for(j=0;j<3;j++)
            {
                xcg[i][j]=0.0; fcg[i][j]=0.0;
            }
...
/*if(i_step==0&&i>106636&& i<=106742) 
{
    ...
    if(j==2)
        ...
        printf("%f\t",x[i][j]);
                    ...
                    fcg[cg_indice[i]-1][j]+=f[i][j]/41.84;
            }
        //print lammps head
        ...
        {
            fprintf(fp_cgmap,"%d %d %f %f %f %f %f %f\n",i+1,cg_types[i],xcg[i][0],xcg[i][1],xcg[i][2],fcg[i][0],fcg[i][1],fcg[i][2]);
        }
    }
